...
export const Menu: React.FC = () => {
  ...
  const categories = [
    { id: 'Breakfast', icon: Sun, time: '07:30 AM — 09:30 AM', color: 'bg-secondary-fixed', textColor: 'text-on-secondary-fixed' },
    ...
    <div className="max-w-[1400px]">
      ...
      <section className="mb-12">
        <div className="flex items-baseline justify-between mb-6">
          ...
          <span className="text-sm font-medium text-on-surface-variant bg-surface-container-low px-4 py-1 rounded-full">
            April 15 — April 21
          </span>
        </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-on-surface-variant">
                    {cat.time} • {MENU_ITEMS.filter(i => i.category === cat.id).length} Options Available
                  </p>
                </div>
              </div>
            ...
            <div className="flex gap-6 overflow-x-auto hide-scrollbar pb-6 -mx-4 px-4">
              {MENU_ITEMS.filter(item => item.category === cat.id).map((item) => (
                <motion.div 
                  key={item.id}
                  whileHover={{ y: -4 }}
                  className="min-w-[700px] flex bg-surface-container-lowest rounded-[2rem] overflow-hidden shadow-[0px_12px_32px_rgba(27,28,25,0.04)] hover:shadow-[0px_12px_32px_rgba(27,28,25,0.08)] transition-all"
                >
...
const NutritionStat = ({ label, value, unit = '', max, color }: { label: string, value: number, unit?: string, max: number, color: string }) => (
